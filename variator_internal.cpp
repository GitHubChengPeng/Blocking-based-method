...
int push(stack *st,int id)
{
     ...
     if (new_el == NULL)
          return(1);
     new_el->next = st->top;    // 第一次创建节点压栈时，st->top尽管没有初始化， 但是因为
								// population类型结构体 包含 stack类型成员"free_ids_stack"，stack结构体 包含 "top"指针成员
								// 而population global_population声明为全局结构体变量， 则所有成员初值默认为0，
								// 即使其stack结构体类型成员free_ids_stack的子成员top也是初始化为0。请参考variator_internal.h文件。
     new_el->identity = id;
     st->top = new_el;
     st->size++;
     return(0);
   ...
   population itself. */
{
     ...
     if (NULL != global_population.individual_array)
     {
		...
		// 并不是删除free_ids_stack本身，而是其中的free_ids_stack.top指针指向的链表被释放空间，
		// 删除最后一个节点时，其next成员（为0）传值给top, 因此top仍被回置为0，为下一次run做准备(如果采用Monitor的话)
