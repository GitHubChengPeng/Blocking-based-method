...
int old_main(int argc, char *argv[])  
{
	...
	//调试用！很管用！
	// strcpy(paramfile,"PPDM_TKDE_04_1.b_param.txt");
	// strcpy(filenamebase, "PISA_");
	// poll = 0.01;
     ...
     write_state(current_state);  //初始状态current_state == 0
   ...
   Returns -1 if adding failed.*/
{
     ...
     {
          global_population.individual_array[identity] = ind;    //将新个体插入种群
     }
     ...
     { 
		  ...
		  // 即使将空间加倍，也应加上将新个体插入
		  // 尽管按照目前的问题规模， 不会出现需要内存空间加倍的情况
		  global_population.individual_array[identity] = ind;   
     }

     return (identity);		//返回个体插入种群的位置（身份证号）
}
...
//根据arc_file文件里的身份证号（个体在种群中位置）保留存档个体， 删除种群中不存档的个体
   ...
   the global population which are not in the arc file. */
{
	 ...
	 //先对要保留的个体的身份证号进行排序（升序），再与种群中的有效个体进行比对， 删除不存档的个体
